<template><h1 id="euserv" tabindex="-1"><a class="header-anchor" href="#euserv" aria-hidden="true">#</a> Euserv</h1>
<p>EUserv是一家德国主机商，提供<a href="https://wzfou.com/vps/" target="_blank" rel="noopener noreferrer">VPS主机<ExternalLinkIcon/></a>、服务器等，目前该商家提供免费ipv6主机服务器，配置信息是CPU：1 Core、内存：1 GB、硬盘：10 GB HDD、带宽：1 Gbit，提供一个IPv6地址，想要IPv4地址那就付费了。虽然<a href="https://wzfou.com/tag/mianfei-vps/" target="_blank" rel="noopener noreferrer">免费VPS<ExternalLinkIcon/></a>主机看起来配置很好，但是用起来非常地卡。</p>
<p>申请<a href="https://wzfou.com/tag/euserv/" target="_blank" rel="noopener noreferrer">EUserv<ExternalLinkIcon/></a>免费VPS主机的过程并不复杂，也不需要什么Paypal、信用卡之类的验证，但是EUserv免费VPS主机申请要人工审核，一般来说48小时内会收到回复。当然，也有申请EUserv<a href="https://wzfou.com/tag/mianfei-vps/" target="_blank" rel="noopener noreferrer">免费VPS<ExternalLinkIcon/></a>主机失败的，或者是EUserv<a href="https://wzfou.com/tag/mianfei-vps/" target="_blank" rel="noopener noreferrer">免费VPS<ExternalLinkIcon/></a>主机被删号的。</p>
<p><img src="https://cdn.jsdelivr.net/gh/xlc520/MyImage/MdImg/EUserv_00.png" alt="img" loading="lazy"></p>
<p>更多的<a href="https://wzfou.com/tag/mianfei-vps/" target="_blank" rel="noopener noreferrer">免费VPS<ExternalLinkIcon/></a>主机，这里还有：</p>
<ol>
<li><a href="https://wzfou.com/oracle-mianfei-vps/" target="_blank" rel="noopener noreferrer">Oracle甲骨文免费云主机申请使用-Oracle免费VPS测评和VPS重置密钥登录<ExternalLinkIcon/></a></li>
<li><a href="https://wzfou.com/aws-vps-pingce/" target="_blank" rel="noopener noreferrer">AWS免费VPS主机申请使用-Amazon EC2韩国日本香港机房VPS主机评测<ExternalLinkIcon/></a></li>
<li><a href="https://wzfou.com/devcloud-apponfly/" target="_blank" rel="noopener noreferrer">英特尔Intel DevCloud免费云服务器和AppOnFly免费试用Windows Server服务器<ExternalLinkIcon/></a></li>
</ol>
<h2 id="一、euserv免费vps申请" tabindex="-1"><a class="header-anchor" href="#一、euserv免费vps申请" aria-hidden="true">#</a> 一、EUserv免费VPS申请</h2>
<p>网站：</p>
<ol>
<li>http://www.euserv.de</li>
...
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code>// 设置 Teredo 服务器，默认为：win10.ipv6.microsoft.com 

netsh interface teredo <span class="token builtin class-name">set</span> state enterpriseclient <span class="token assign-left variable">server</span><span class="token operator">=</span>default 

// 测试 IPv6 连接 

<span class="token function">ping</span> -6 ipv6.test-ipv6.com 

<span class="token function">ping</span> -6 <span class="token punctuation">[</span><span class="token number">2001</span>:470:1:18::125<span class="token punctuation">]</span> 
...
netsh interface teredo <span class="token builtin class-name">set</span> state <span class="token assign-left variable">server</span><span class="token operator">=</span>teredo.remlab.net 

// 第二种：先卸载当前 Teredo 适配器再重新启用 

netsh interface Teredo <span class="token builtin class-name">set</span> state disable 

netsh interface Teredo <span class="token builtin class-name">set</span> state <span class="token assign-left variable">type</span><span class="token operator">=</span>default 
...
<p>宝塔面板后台默认的是不支持IPv6的，我们需要手动开启：</p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code><span class="token punctuation">\</span>#让宝塔面板监听IPv6

<span class="token builtin class-name">echo</span> “**True**” <span class="token operator">></span> /www/server/panel/data/ipv6.pl

<span class="token punctuation">\</span>#如果发现端口无法使用，也可以直接修改端口

<span class="token builtin class-name">echo</span> “8888” <span class="token operator">></span> /www/server/panel/data/port.pl

<span class="token punctuation">\</span>#重启面板：

bt restart

<span class="token punctuation">\</span>#宝塔面板在面板设置里勾选了监听ipv6，正常情况下ipv4应该也是可以使用的，部分服务器出现问题的话会导致ipv4无法登陆。使用以下命令

<span class="token function">rm</span> -f /www/server/panel/data/ipv6.pl <span class="token operator">&amp;&amp;</span> /etc/init.d/bt restart
</code></pre><div class="line-numbers" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br><span class="line-number">12</span><br><span class="line-number">13</span><br><span class="line-number">14</span><br><span class="line-number">15</span><br></div></div><p>如果是Oneinstack或者LNMP一键安装包，搭建网站还需要修改一下Nginx的配置，让Nginx同时监听IPv4和IPv6的请求，示例如下：</p>
<div class="language-bash ext-sh line-numbers-mode"><pre v-pre class="language-bash"><code> listen <span class="token number">80</span><span class="token punctuation">;</span>
...
​    server_name www.wzfou.com<span class="token punctuation">;</span>
...
<h2 id="六、用腾讯云函数给euserv德国免费vps自动续期" tabindex="-1"><a class="header-anchor" href="#六、用腾讯云函数给euserv德国免费vps自动续期" aria-hidden="true">#</a> 六、用腾讯云函数给EUserv德国免费VPS自动续期</h2>
<p>使用腾讯云函数 SCF自动续期EUserv免费IPv6 VPS脚本见 https://github.com/o0oo0ooo0/EUserv_extend</p>
...
<p>PASSWORD: 账户的密码</p>
<p>PASSWORD = ‘password’
PASSWORD = ‘password1 password2’ # 多个账号写法</p>
<p><strong>设置通知提醒（可选操作）</strong></p>
<p>我已Server酱的提醒为例，打开https://sct.ftqq.com后微信扫码获取sentkey， 然后填写在上面函数代码Server酱的key处。如果您不需要通知也可以不设置提醒。这步不用设置。</p>
