...
- `C`
  ...
  如果指定的文件或目录不存在或目录为空， 那么递归的从"参数"字段所指定的文件或目录复制， 否则什么也不做。
...
可以同时使用多个时间单位，例如"5m10s"表示 5 分 10 秒(也就是 310 秒)。如果省略了时间单位，那么表示使用默认单位"秒"。
...
让我们配置 systemd-tmpfiles 来清理/mytmp 目录，这将确保目录不包含最近 3 天未使用过的文件。
