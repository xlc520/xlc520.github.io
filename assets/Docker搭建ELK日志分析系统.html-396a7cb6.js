import{_ as i}from"./plugin-vue_export-helper-c27b6911.js";import{r as l,o,c,b as n,d as s,e,a as t}from"./app-4a6db5be.js";const p={},u=t(`<h1 id="docker搭建elk日志分析系统" tabindex="-1"><a class="header-anchor" href="#docker搭建elk日志分析系统" aria-hidden="true">#</a> Docker搭建ELK日志分析系统</h1><h2 id="方法一" tabindex="-1"><a class="header-anchor" href="#方法一" aria-hidden="true">#</a> 方法一</h2><ul><li><p>ElasticSearch 有强大的搜索功能的无模式数据库，可以简单的很想扩展，索引每一个字段，可以聚合分组数据。</p></li><li><p>Logstash 用Ruby编写的，我们可以使用管道输入和输出数据到任何位置。一个可以抓取，转换，存储事件到ElasticSearch的ETL管道。打包版本在JRuby上运行，并使用几十个线程进行并行的数据处理，利用了JVM的线程功能。</p></li><li><p>Kibana 基于web的数据分析，为ElasticSearch仪表板的工具。充分利用ElasticSearch的搜索功能，以秒为单位可视化数据。支持Lucene的查询字符串的语法和Elasticsearch的过滤功能。</p></li></ul><h3 id="前提" tabindex="-1"><a class="header-anchor" href="#前提" aria-hidden="true">#</a> 前提</h3><ul><li>本文中架构基于docker搭建，需要您了解docker的基本概念，基本操作和docker1.9之后的自定义overlay网络</li></ul><blockquote><p>本文只介绍了最简化搭建。如果您用于生产，还需要在如下方面完善 1.elastic是有存储目录，需要在docker中进行数据卷映射。配置文件elasticsearch.yml需要根据自己需求自行配置。请参考：https://hub.docker.com/_/elasticsearch/ 2.Dockerhub官方提供的镜像基于不同的基础镜像，不利于网络传输！建议根据自己组织内部镜像重新创建！</p></blockquote><h3 id="docker搭建elk的javaweb应用日志收集存储分析系统" tabindex="-1"><a class="header-anchor" href="#docker搭建elk的javaweb应用日志收集存储分析系统" aria-hidden="true">#</a> Docker搭建ELK的javaweb应用日志收集存储分析系统</h3><h3 id="第一步-启动elasticsearch" tabindex="-1"><a class="header-anchor" href="#第一步-启动elasticsearch" aria-hidden="true">#</a> 第一步：启动elasticsearch</h3><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>docker run -d --name myes  \\
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>将密码妥善保存后，我们需要将 <code>docker-compose.yml</code> 配置文件中的 elasticsearch 服务的 <code>ELASTIC_PASSWORD</code> 去掉，这样可以确保服务启动只使用我们刚刚重置后的密码（keystore）。以及需要对 kibana 、 logstash 配置文件中的信息进行替换，将文件中的 <strong>elastic</strong> 用户的密码进行更新，相关文件我们在开篇的目录结构中有提过，暂时先修改下面三个文件就可以了：</p><ul><li>kibana/config/kibana.yml</li><li>logstash/config/logstash.yml</li><li>logstash/pipeline/logstash.conf</li></ul>`,47),r={href:"https://www.elastic.co/guide/en/logstash/current/ls-security.html",target:"_blank",rel:"noopener noreferrer"},d=t(`<p>在配置修改完毕后，我们执行 <code>docker-compose restart</code> 重启相关服务。</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token function">docker-compose</span> restart
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div>`,19),m={href:"https://www.elastic.co/guide/en/elasticsearch/reference/current/heap-size.html",target:"_blank",rel:"noopener noreferrer"},b=t(`<div class="language-yaml line-numbers-mode" data-ext="yml"><pre class="language-yaml"><code><span class="token key atrule">environment</span><span class="token punctuation">:</span>
  <span class="token key atrule">ES_JAVA_OPTS</span><span class="token punctuation">:</span> <span class="token string">&quot;-Xmx1g -Xms1g&quot;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>如果你真实使用在生产环节，务必开启 TLS 和密码认证，此处为了不额外扩展篇幅（以及懒得申请通配符证书/配置自签名）先使用关闭安全配置的方式忽略这个设置 : )</p><h3 id="修改配置支持多实例" tabindex="-1"><a class="header-anchor" href="#修改配置支持多实例" aria-hidden="true">#</a> 修改配置支持多实例</h3>`,3),g={href:"https://www.elastic.co/guide/en/elasticsearch/reference/current/docker.html",target:"_blank",rel:"noopener noreferrer"},h=t(`<p>首先创建服务所需要的数据目录，并赋予所需要的权限。</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token function">mkdir</span> <span class="token parameter variable">-p</span> data/<span class="token punctuation">{</span>es01,es02,es03<span class="token punctuation">}</span>
<span class="token function">chmod</span> g+rwx data/*
<span class="token function">chgrp</span> <span class="token number">0</span> data/*
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,2),y={href:"https://docs.docker.com/engine/extend/legacy_plugins/",target:"_blank",rel:"noopener noreferrer"},q=t(`<div class="language-yaml line-numbers-mode" data-ext="yml"><pre class="language-yaml"><code><span class="token key atrule">volumes</span><span class="token punctuation">:</span>
