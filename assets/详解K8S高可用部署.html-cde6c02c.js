...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653044-1.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>查看日志，发现有报错，报错如下：</p><blockquote><p>kubelet.service: Main process exited, code=exited, status=1/FAILURE kubelet.service: Failed with result &#39;exit-code&#39;.</p></blockquote><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653045-2.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><blockquote><p>【解释】重新安装（或第一次安装）k8s，未经过 kubeadm init 或者 kubeadm join 后，kubelet 会不断重启，这个是正常现象……，执行 init 或 join 后问题会自动解决，对此官网有如下描述，也就是此时不用理会 kubelet.service。</p></blockquote><p>查看版本</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>kubectl version
yum info kubeadm
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653045-3.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><h4 id="_7-使用-kubeadm-初始化集群-master-节点" tabindex="-1"><a class="header-anchor" href="#_7-使用-kubeadm-初始化集群-master-节点" aria-hidden="true">#</a> 7）使用 kubeadm 初始化集群（master 节点）</h4><p>最好提前把镜像下载好，这样安装快</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>docker pull registry.aliyuncs.com/google_containers/kube-apiserver:v1.24.1
...
source  ~/.bash_profile
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653045-4.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>发现节点还是有问题，查看日志 /var/log/messages</p><blockquote><p>&quot;Container runtime network not ready&quot; networkReady=&quot;NetworkReady=false reason:NetworkPluginNotReady message:Network plugin returns error: cni plugin not initialized&quot;</p></blockquote><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653045-5.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>接下来就是安装 Pod 网络插件</p><h4 id="_8-安装-pod-网络插件-cni-container-network-interface-master" tabindex="-1"><a class="header-anchor" href="#_8-安装-pod-网络插件-cni-container-network-interface-master" aria-hidden="true">#</a> 8）安装 Pod 网络插件（CNI：Container Network Interface）(master)</h4><p>你必须部署一个基于 Pod 网络插件的 容器网络接口 (CNI)，以便你的 Pod 可以相互通信。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code># 最好提前下载镜像（所有节点）
docker pull quay.io/coreos/flannel:v0.14.0
kubectl apply -f https://raw.githubusercontent.com/coreos/flannel/master/Documentation/kube-flannel.yml
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果上面安装失败，则下载我百度里的，离线安装</p>`,63),b={href:"https://pan.baidu.com/s/1HB9xuO3bssAW7v5HzpXkeQ",target:"_blank",rel:"noopener noreferrer"},p=d(`<p>再查看 node 节点，就已经正常了</p><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653045-6.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><h4 id="_9-node-节点加入-k8s-集群" tabindex="-1"><a class="header-anchor" href="#_9-node-节点加入-k8s-集群" aria-hidden="true">#</a> 9）node 节点加入 k8s 集群</h4><p>先安装 kubelet</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>yum install -y kubelet kubeadm kubectl --disableexcludes=kubernetes
# 设置为开机自启并现在立刻启动服务 --now：立刻启动服务
systemctl enable --now kubelet
systemctl status kubelet
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653045-8.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>4、重启 kube-proxy</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code># 先查看
kubectl get pod -n kube-system | grep kube-proxy
# 再delete让它自拉起
kubectl get pod -n kube-system | grep kube-proxy |awk &#39;{system(&quot;kubectl delete pod &quot;$1&quot; -n kube-system&quot;)}&#39;
# 再查看
kubectl get pod -n kube-system | grep kube-proxy
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653045-9.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>5、查看 ipvs 转发规则</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>ipvsadm -Ln
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653045-10.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><h4 id="_11-集群高可用配置" tabindex="-1"><a class="header-anchor" href="#_11-集群高可用配置" aria-hidden="true">#</a> 11）集群高可用配置</h4><p>配置高可用（HA）Kubernetes 集群实现的两种方案：</p><p>使用堆叠（stacked）控制平面节点，其中 etcd 节点与控制平面节点共存（本章使用），架构图如下：</p><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653045-11.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>使用外部 etcd 节点，其中 etcd 在与控制平面不同的节点上运行，架构图如下：</p><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653045-12.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>这里新增一台机器作为另外一个 master 节点：192.168.0.116 配置跟上面 master 节点一样。只是不需要最后一步初始化了。</p><p>1、修改主机名和配置 hosts</p><p>所有节点都统一如下配置：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code># 在192.168.0.113执行
hostnamectl set-hostname  k8s-master-168-0-113
# 在192.168.0.114执行
hostnamectl set-hostname k8s-node1-168-0-114
# 在192.168.0.115执行
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-13.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>根据提示执行如下命令：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>mkdir -p $HOME/.kube
sudo cp -i /etc/kubernetes/admin.conf $HOME/.kube/config
sudo chown $(id -u):$(id -g) $HOME/.kube/config
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-14.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>虽然现在已经有两个 master 了，但是对外还是只能有一个入口的，所以还得要一个负载均衡器，如果一个 master 挂了，会自动切到另外一个 master 节点。</p><h4 id="_12-部署-nginx-keepalived-高可用负载均衡器" tabindex="-1"><a class="header-anchor" href="#_12-部署-nginx-keepalived-高可用负载均衡器" aria-hidden="true">#</a> 12）部署 Nginx+Keepalived 高可用负载均衡器</h4><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-15.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>1、安装 Nginx 和 Keepalived</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code># 在两个master节点上执行
...
EOF
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>vrrp_script：指定检查 nginx 工作状态脚本（根据 nginx 状态判断是否故障转移） virtual_ipaddress：虚拟 IP（VIP）</p><p>检查 nginx 状态脚本：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>cat &gt; /etc/keepalived/check_nginx.sh  &lt;&lt; &quot;EOF&quot;
#!/bin/bash
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-16.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>6、修改 hosts（所有节点）</p><p>将 cluster-endpoint 之前执行的 ip 修改执行现在的 VIP</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>192.168.0.113 k8s-master-168-0-113
192.168.0.114 k8s-node1-168-0-114
192.168.0.115 k8s-node2-168-0-115
192.168.0.116 k8s-master2-168-0-116
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-17.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>高可用测试验证，将 k8s-master-168-0-113 节点关机</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>shutdown -h now
curl -k https://cluster-endpoint:16443/version
kubectl get nodes -A
kubectl get pods -A
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><blockquote><p>【温馨提示】堆叠集群存在耦合失败的风险。如果一个节点发生故障，则 etcd 成员和控制平面实例都将丢失， 并且冗余会受到影响。你可以通过添加更多控制平面节点来降低此风险。另外，搜索公众号编程技术圈后台回复“1024”，获取一份惊喜礼包。</p></blockquote><h3 id="三、k8s-管理平台-dashboard-环境部署" tabindex="-1"><a class="header-anchor" href="#三、k8s-管理平台-dashboard-环境部署" aria-hidden="true">#</a> 三、k8s 管理平台 dashboard 环境部署</h3><h4 id="_1-dashboard-部署" tabindex="-1"><a class="header-anchor" href="#_1-dashboard-部署" aria-hidden="true">#</a> 1）dashboard 部署</h4>`,116),g={href:"https://github.com/kubernetes/dashboard",target:"_blank",rel:"noopener noreferrer"},h=d(`<div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>kubectl apply -f https://raw.githubusercontent.com/kubernetes/dashboard/v2.6.0/aio/deploy/recommended.yaml
  ...
  template:
    ...
    spec:
      ...
      volumes:
        - name: tmp-volume
          emptyDir: {}
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-18.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>重新部署</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>kubectl delete -f recommended.yaml
kubectl apply -f recommended.yaml
kubectl get svc,pods -n kubernetes-dashboard
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-19.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><h4 id="_2-创建登录用户" tabindex="-1"><a class="header-anchor" href="#_2-创建登录用户" aria-hidden="true">#</a> 2）创建登录用户</h4><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>cat &gt;ServiceAccount.yaml&lt;&lt;EOF
apiVersion: v1
kind: ServiceAccount
metadata:
  name: admin-user
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div>`,15),x={href:"https://cluster-endpoint:31443",target:"_blank",rel:"noopener noreferrer"},k=e("figure",null,[e("img",{src:"http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-20.png",alt:"K8S高可用部署",tabindex:"0",loading:"lazy"}),e("figcaption",null,"K8S高可用部署")],-1),f=e("p",null,"输入上面创建的 token 登录",-1),y=e("figure",null,[e("img",{src:"http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-21.png",alt:"K8S高可用部署",tabindex:"0",loading:"lazy"}),e("figcaption",null,"K8S高可用部署")],-1),_=e("h3",{id:"四、k8s-镜像仓库-harbor-环境部署",tabindex:"-1"},[e("a",{class:"header-anchor",href:"#四、k8s-镜像仓库-harbor-环境部署","aria-hidden":"true"},"#"),n(" 四、k8s 镜像仓库 harbor 环境部署")],-1),q={href:"https://github.com/helm/helm/releases",target:"_blank",rel:"noopener noreferrer"},S=d(`<h4 id="_1-安装-helm" tabindex="-1"><a class="header-anchor" href="#_1-安装-helm" aria-hidden="true">#</a> 1）安装 helm</h4><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>mkdir -p /opt/k8s/helm &amp;&amp; cd /opt/k8s/helm
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_4-安装-ingress" tabindex="-1"><a class="header-anchor" href="#_4-安装-ingress" aria-hidden="true">#</a> 4）安装 ingress</h3>`,7),w={href:"https://kubernetes.github.io/ingress-nginx/",target:"_blank",rel:"noopener noreferrer"},C={href:"https://github.com/kubernetes/ingress-nginx",target:"_blank",rel:"noopener noreferrer"},K={href:"https://kubernetes.github.io/ingress-nginx/deploy/",target:"_blank",rel:"noopener noreferrer"},I=d(`<p>1、通过 helm 部署</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>helm upgrade --install ingress-nginx ingress-nginx \\
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-22.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><h4 id="_5-安装-nfs" tabindex="-1"><a class="header-anchor" href="#_5-安装-nfs" aria-hidden="true">#</a> 5）安装 nfs</h4><p>1、所有节点安装 nfs</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>yum -y install  nfs-utils rpcbind
...
# 配置生效
exportfs -r
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>exportfs 命令</p><blockquote><p>常用选项 -a 全部挂载或者全部卸载 -r 重新挂载 -u 卸载某一个目录 -v 显示共享目录 以下操作在服务端上</p></blockquote><p>4、启动 rpc 和 nfs（客户端只需要启动 rpc 服务）（注意顺序）</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>systemctl start rpcbind
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><blockquote><p>-e 显示 NFS 服务器的共享列表 -a 显示本机挂载的文件资源的情况 NFS 资源的情况 -v 显示版本号</p></blockquote><p>5、客户端</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code># 安装
...
yum -y install rsync
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>【2】配置 在/etc/rsyncd.conf 中添加</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>cat &gt;/etc/rsyncd.conf&lt;&lt;EOF
uid = root
gid = root
#禁锢在源目录
use chroot = yes
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><blockquote><p>【温馨提示】crontab 定时同步数据不太好，可以使用rsync+inotify做数据实时同步，这里篇幅有点长了，先不讲，如果后面有时间会出一篇单独文章来讲。另外，搜索公众号后端架构师后台回复“架构整洁”，获取一份惊喜礼包。</p></blockquote><h3 id="_6-创建-nfs-provisioner-和持久化存储-sc" tabindex="-1"><a class="header-anchor" href="#_6-创建-nfs-provisioner-和持久化存储-sc" aria-hidden="true">#</a> 6）创建 nfs provisioner 和持久化存储 SC</h3><blockquote><p>【温馨提示】这里跟我之前的文章有点不同，之前的方式也不适用新版本。</p></blockquote>`,42),E={href:"https://github.com/kubernetes-sigs/nfs-subdir-external-provisioner",target:"_blank",rel:"noopener noreferrer"},O=d(`<p>helm 部署 nfs-subdir-external-provisioner</p><p>1、添加 helm 仓库</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>helm repo add nfs-subdir-external-provisioner https://kubernetes-sigs.github.io/nfs-subdir-external-provisioner/
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-23.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><h4 id="_7-部署-harbor-https-方式" tabindex="-1"><a class="header-anchor" href="#_7-部署-harbor-https-方式" aria-hidden="true">#</a> 7）部署 Harbor（Https 方式）</h4><p>1、创建 Namespace</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>kubectl create ns harbor
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-24.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><h3 id="_5、ingress-没有-address-问题解决" tabindex="-1"><a class="header-anchor" href="#_5、ingress-没有-address-问题解决" aria-hidden="true">#</a> 5、ingress 没有 ADDRESS 问题解决</h3><p>【分析】，发现&quot;error: endpoints “default-http-backend” not found&quot;</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>cat &lt;&lt; EOF &gt; default-http-backend.yaml
---
...
helm install myharbor --namespace harbor harbor/harbor \\
  ...
  --set externalURL=https://myharbor.com \\
  --set harborAdminPassword=Harbor12345
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-25.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>5、访问 harbor</p>`,29),N={href:"https://myharbor.com",target:"_blank",rel:"noopener noreferrer"},z=d(`<figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-26.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>6、harbor 常见操作 【1】创建项目 bigdata</p><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-27.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>【2】配置私有仓库 在文件/etc/docker/daemon.json添加如下内容：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>&quot;insecure-registries&quot;:[&quot;https://myharbor.com&quot;]
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653047-28.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>【4】打标签并把镜像上传到 harbor</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>docker tag rancher/pause:3.6 myharbor.com/bigdata/pause:3.6
docker push myharbor.com/bigdata/pause:3.6
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>7、修改 containerd 配置</p><p>以前使用 docker-engine 的时候，只需要修改/etc/docker/daemon.json 就行，但是新版的 k8s 已经使用 containerd 了，所以这里需要做相关配置，要不然 containerd 会失败。证书（ca.crt）可以在页面上下载：</p><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653047-29.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>创建域名目录</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>mkdir /etc/containerd/myharbor.com
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653047-30.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>重启 containerd</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>#重新加载配置
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><figure><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653047-31.png" alt="K8S高可用部署" tabindex="0" loading="lazy"><figcaption>K8S高可用部署</figcaption></figure><p>Kubernetes（k8s）最新版最完整版基础环境部署+master 高可用实现详细步骤就到这里了</p>`,32);function P(A,F){const i=t("ExternalLinkIcon");return l(),r("div",null,[v,e("p",null,[n("官网："),e("a",u,[n("https://kubernetes.io/"),s(i)]),n(" 官方文档："),e("a",o,[n("https://kubernetes.io/zh-cn/docs/home/"),s(i)])]),m,e("blockquote",null,[e("p",null,[n("链接："),e("a",b,[n("https://pan.baidu.com/s/1HB9xuO3bssAW7v5HzpXkeQ"),s(i)]),n(" 提取码：8888")])]),p,e("p",null,[n("GitHub 地址："),e("a",g,[n("https://github.com/kubernetes/dashboard"),s(i)])]),h,e("p",null,[n("登录："),e("a",x,[n("https://cluster-endpoint:31443"),s(i)])]),k,f,y,_,e("p",null,[n("GitHub 地址："),e("a",q,[n("https://github.com/helm/helm/releases"),s(i)]),n(" 这使用 helm 安装，所以得先安装 helm。另外，搜索公众号Java架构师技术后台回复“面试题”，获取一份惊喜礼包。")]),S,e("p",null,[n("ingress 官方网站："),e("a",w,[n("https://kubernetes.github.io/ingress-nginx/"),s(i)]),n(" ingress 仓库地址："),e("a",C,[n("https://github.com/kubernetes/ingress-nginx"),s(i)]),n(" 部署文档："),e("a",K,[n("https://kubernetes.github.io/ingress-nginx/deploy/"),s(i)])]),I,e("p",null,[n("GitHub 地址："),e("a",E,[n("https://github.com/kubernetes-sigs/nfs-subdir-external-provisioner"),s(i)])]),O,e("p",null,[e("a",N,[n("https://myharbor.com"),s(i)]),n(" 账号/密码：admin/Harbor12345")]),z])}const R=a(c,[["render",P],["__file","详解K8S高可用部署.html.vue"]]);export{R as default};
