...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果上面安装失败，则下载我百度里的，离线安装</p>`,63),b={href:"https://pan.baidu.com/s/1HB9xuO3bssAW7v5HzpXkeQ",target:"_blank",rel:"noopener noreferrer"},p=d(`<p>再查看 node 节点，就已经正常了</p><p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653045-6.png" alt="K8S高可用部署" loading="lazy"></p><h4 id="_9-node-节点加入-k8s-集群" tabindex="-1"><a class="header-anchor" href="#_9-node-节点加入-k8s-集群" aria-hidden="true">#</a> 9）node 节点加入 k8s 集群</h4><p>先安装 kubelet</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>yum install -y kubelet kubeadm kubectl --disableexcludes=kubernetes
# 设置为开机自启并现在立刻启动服务 --now：立刻启动服务
systemctl enable --now kubelet
systemctl status kubelet
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653045-8.png" alt="K8S高可用部署" loading="lazy"></p><p>4、重启 kube-proxy</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code># 先查看
kubectl get pod -n kube-system | grep kube-proxy
# 再delete让它自拉起
kubectl get pod -n kube-system | grep kube-proxy |awk &#39;{system(&quot;kubectl delete pod &quot;$1&quot; -n kube-system&quot;)}&#39;
# 再查看
kubectl get pod -n kube-system | grep kube-proxy
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653045-9.png" alt="K8S高可用部署" loading="lazy"></p><p>5、查看 ipvs 转发规则</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>ipvsadm -Ln
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-13.png" alt="K8S高可用部署" loading="lazy"></p><p>根据提示执行如下命令：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>mkdir -p $HOME/.kube
sudo cp -i /etc/kubernetes/admin.conf $HOME/.kube/config
sudo chown $(id -u):$(id -g) $HOME/.kube/config
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-14.png" alt="K8S高可用部署" loading="lazy"></p><p>虽然现在已经有两个 master 了，但是对外还是只能有一个入口的，所以还得要一个负载均衡器，如果一个 master 挂了，会自动切到另外一个 master 节点。</p><h4 id="_12-部署-nginx-keepalived-高可用负载均衡器" tabindex="-1"><a class="header-anchor" href="#_12-部署-nginx-keepalived-高可用负载均衡器" aria-hidden="true">#</a> 12）部署 Nginx+Keepalived 高可用负载均衡器</h4><p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-15.png" alt="K8S高可用部署" loading="lazy"></p><p>1、安装 Nginx 和 Keepalived</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code># 在两个master节点上执行
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-16.png" alt="K8S高可用部署" loading="lazy"></p><p>6、修改 hosts（所有节点）</p><p>将 cluster-endpoint 之前执行的 ip 修改执行现在的 VIP</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>192.168.0.113 k8s-master-168-0-113
192.168.0.114 k8s-node1-168-0-114
192.168.0.115 k8s-node2-168-0-115
192.168.0.116 k8s-master2-168-0-116
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-17.png" alt="K8S高可用部署" loading="lazy"></p><p>高可用测试验证，将 k8s-master-168-0-113 节点关机</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>shutdown -h now
  ...
  template:
    ...
    spec:
      ...
      volumes:
        - name: tmp-volume
          emptyDir: {}
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-18.png" alt="K8S高可用部署" loading="lazy"></p><p>重新部署</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>kubectl delete -f recommended.yaml
kubectl apply -f recommended.yaml
kubectl get svc,pods -n kubernetes-dashboard
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-19.png" alt="K8S高可用部署" loading="lazy"></p><h4 id="_2-创建登录用户" tabindex="-1"><a class="header-anchor" href="#_2-创建登录用户" aria-hidden="true">#</a> 2）创建登录用户</h4><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>cat &gt;ServiceAccount.yaml&lt;&lt;EOF
apiVersion: v1
kind: ServiceAccount
metadata:
  name: admin-user
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div>`,15),x={href:"https://cluster-endpoint:31443",target:"_blank",rel:"noopener noreferrer"},k=e("p",null,[e("img",{src:"http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-20.png",alt:"K8S高可用部署",loading:"lazy"})],-1),y=e("p",null,"输入上面创建的 token 登录",-1),f=e("p",null,[e("img",{src:"http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-21.png",alt:"K8S高可用部署",loading:"lazy"})],-1),_=e("h3",{id:"四、k8s-镜像仓库-harbor-环境部署",tabindex:"-1"},[e("a",{class:"header-anchor",href:"#四、k8s-镜像仓库-harbor-环境部署","aria-hidden":"true"},"#"),n(" 四、k8s 镜像仓库 harbor 环境部署")],-1),q={href:"https://github.com/helm/helm/releases",target:"_blank",rel:"noopener noreferrer"},S=d(`<h4 id="_1-安装-helm" tabindex="-1"><a class="header-anchor" href="#_1-安装-helm" aria-hidden="true">#</a> 1）安装 helm</h4><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>mkdir -p /opt/k8s/helm &amp;&amp; cd /opt/k8s/helm
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-22.png" alt="K8S高可用部署" loading="lazy"></p><h4 id="_5-安装-nfs" tabindex="-1"><a class="header-anchor" href="#_5-安装-nfs" aria-hidden="true">#</a> 5）安装 nfs</h4><p>1、所有节点安装 nfs</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>yum -y install  nfs-utils rpcbind
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-23.png" alt="K8S高可用部署" loading="lazy"></p><h4 id="_7-部署-harbor-https-方式" tabindex="-1"><a class="header-anchor" href="#_7-部署-harbor-https-方式" aria-hidden="true">#</a> 7）部署 Harbor（Https 方式）</h4><p>1、创建 Namespace</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>kubectl create ns harbor
...
helm install myharbor --namespace harbor harbor/harbor \\
  ...
  --set externalURL=https://myharbor.com \\
  --set harborAdminPassword=Harbor12345
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-25.png" alt="K8S高可用部署" loading="lazy"></p><p>5、访问 harbor</p>`,29),z={href:"https://myharbor.com",target:"_blank",rel:"noopener noreferrer"},P=d(`<p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-26.png" alt="K8S高可用部署" loading="lazy"></p><p>6、harbor 常见操作 【1】创建项目 bigdata</p><p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653046-27.png" alt="K8S高可用部署" loading="lazy"></p><p>【2】配置私有仓库 在文件/etc/docker/daemon.json添加如下内容：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>&quot;insecure-registries&quot;:[&quot;https://myharbor.com&quot;]
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653047-28.png" alt="K8S高可用部署" loading="lazy"></p><p>【4】打标签并把镜像上传到 harbor</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>docker tag rancher/pause:3.6 myharbor.com/bigdata/pause:3.6
docker push myharbor.com/bigdata/pause:3.6
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>7、修改 containerd 配置</p><p>以前使用 docker-engine 的时候，只需要修改/etc/docker/daemon.json 就行，但是新版的 k8s 已经使用 containerd 了，所以这里需要做相关配置，要不然 containerd 会失败。证书（ca.crt）可以在页面上下载：</p><p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653047-29.png" alt="K8S高可用部署" loading="lazy"></p><p>创建域名目录</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>mkdir /etc/containerd/myharbor.com
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><img src="http://122.9.159.116:5244/d/ecloud180/images/blogImage/640-1667642653047-30.png" alt="K8S高可用部署" loading="lazy"></p><p>重启 containerd</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>#重新加载配置
